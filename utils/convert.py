...
def process_docx_file(
    ...
    convert_doc_to_json(working_directory, json_path)
    ...
    for para in doc.paragraphs:
        ...
        if new_para and current_song:
            if is_collapsible:
                ...
                data[-1]["defaultContent"]["items"].append(
                    {
                        "type": current_type,
                        "content": current_song,
                    }
        ...
        if match:
            ...
            if current_type == "section":
                ...
                is_collapsible = False
            elif current_type == "enddynamicsong":
                dynamic_song = False
                ...
                button_contents = content.split("|")
                if len(button_contents) >= 3:
                    link, replace, label = button_contents
                    data.append(
                        {
                            "type": current_type,
                            "link": link.strip(),
                            "replace": (
                                True if replace.strip().lower() == "true" else False
                            ),
                            "label": label.strip(),
                        }
                    )
                elif len(button_contents) == 2:
                    link, replace = button_contents
                    data.append(
                        {
                            "type": current_type,
                            "link": link.strip(),
                    ...
                    data.append(
                        {
                            "type": current_type,
                            "link": button_contents[0].strip(),
                        }
                    )
            elif current_type == "dynamicsong":
                timeKey, title = content.split("|")
                data.append(
                    {
                        "type": "dynamic-songs-block",
                        "timeKey": timeKey.strip(),
                        "defaultContent": {
                            "eventKey": "default",
                            "eventTitle": title.strip(),
                            "timeKey": timeKey.strip(),
                            "items": [],
                        },
                    }
